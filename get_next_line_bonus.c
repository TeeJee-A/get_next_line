...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/17 18:11:03 by ataji             #+#    #+#             */
/*   Updated: 2022/01/20 01:03:46 by ataji            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_line_file(char **static_line, int fd)
{
	...
	if (!store_line)
		return (NULL);
	while (!get_back_slash_n(static_line[fd]))
	{
		...
		static_line[fd] = ft_strjoin(static_line[fd], store_line);
	}
	if (store_line)
		free(store_line);
	return (static_line[fd]);
}
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	static_line[fd] = get_line_file(static_line, fd);
	if (!static_line[fd] || (static_line[fd] && *static_line[fd] == '\0'))
		return (NULL);
	line = get_line_static(static_line[fd]);
	static_line[fd] = rest_line(static_line[fd]);
	return (line);
}
